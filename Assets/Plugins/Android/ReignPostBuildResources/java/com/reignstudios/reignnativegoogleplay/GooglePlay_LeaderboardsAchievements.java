...
public class GooglePlay_LeaderboardsAchievements implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, ReignActivityCallbacks
{
	...
	public static void Init(boolean disableUsernameRetrieval)
	{
		...
		ReignAndroidPlugin.RootActivity.runOnUiThread(new Runnable()
		{
			public void run()
			{
				try
				{
					GoogleApiClient.Builder builder = new GoogleApiClient.Builder(ReignAndroidPlugin.RootActivity, singleton, singleton);
					...
					builder.addApi(Games.API)
			           ...
			           .setViewForPopups(ReignAndroidPlugin.ContentView);
	...
	public static void ShowNativeAchievementsPage()
	{
		ReignAndroidPlugin.RootActivity.runOnUiThread(new Runnable()
		{
			...
			{
				if (!checkAuthenticationState()) return;
				ReignAndroidPlugin.RootActivity.startActivityForResult(Games.Achievements.getAchievementsIntent(client), REQUEST_ACHIEVEMENTS_ID);
			}
		});
	}
	...
	public static void ShowNativeScoresPage(final String id)
	{
		ReignAndroidPlugin.RootActivity.runOnUiThread(new Runnable()
		{
			...
			{
				if (!checkAuthenticationState()) return;
				ReignAndroidPlugin.RootActivity.startActivityForResult(Games.Leaderboards.getLeaderboardIntent(client, id), REQUEST_LEADERBOARD_ID);
			}
		});
	}
	...
	public void onConnectionFailed(ConnectionResult arg0)
	{
		...
		try
		{
			arg0.startResolutionForResult(ReignAndroidPlugin.RootActivity, RC_RESOLVE_ID);
		}
